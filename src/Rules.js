...
//[1,2,2,1,2]
//another solution:
//freqs.includes(2)&&freq.includes(3)
class FullHouse extends Rule {
  evalRoll = dice => {
    return _.isEqual(this.freq(dice).sort((a, b) => a - b), [2, 3]) ? this.score : 0;
  }
}
...
class SmallStraight extends Rule {
  ...
  evalRoll = dice => {
    ...
    const fiveInRow = (d.size === 5) && (!d.has(1) || !d.has(6))
    //1234 || 2345 || 3456 => 234+1/5 + 345+2/6
    const fourInRow = (d.has(2) && d.has(3) && d.has(4)) && (d.has(1) || d.has(5)) || ((d.has(3) && d.has(4) && d.has(5)) && (d.has(2) || d.has(6)))
    // large straight must be 5 different dice & only one can be a 1 or a 6
    return (fiveInRow || fourInRow) ? this.score : 0;
  };
}
...
class LargeStraight extends Rule {
  evalRoll = dice => {
    ...
    // large straight must be 5 different dice & only one can be a 1 or a 6
    //12345 do not have 6, 23456 do not have 1
